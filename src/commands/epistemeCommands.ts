...
import { generateQuizFeedback, generateQuizQuestions, generateSnippetExplanation } from '../utils/queryHelpers';
import { 
  getQuizFeedbackHTML,
  ...
  export async function understandWithEpisteme(): Promise<void> {
    ...
    if (!editor || editor.selection.isEmpty) {
      vscode.window.showInformationMessage("Please select some code to understand.");
      return;
    }
    ...
    panel.webview.onDidReceiveMessage(async (message) => {
      if (message.type === 'submitUnderstanding') {
        ...
        const explanation = await generateSnippetExplanation(selectedCode, userInput);
    ...
    function waitForQuizFocus(panel: vscode.WebviewPanel): Promise<string | undefined> {
  ...
  function waitForSelection(panel: vscode.WebviewPanel): Promise<number | undefined> {
    return new Promise((resolve) => {
      const subscription = panel.webview.onDidReceiveMessage((message) => {
  ...
  export async function quizWithEpisteme(): Promise<void> {
    ...
    const responses: { question: string; selectedOption: string; correct: boolean }[] = [];
    ...
    async function showNextQuestion(): Promise<void> {
      ...
      if (selection !== undefined) {
        ...
        responses.push({ question: currentQuestion.question, selectedOption, correct: isCorrect });
        if (isCorrect) {
          correctCount++;
        }
      }
